...
def build_stage1_step_prompt(
    ...
    return dedent(
        ...
        - 信息约束：
          - **必须完整保留解题所需的判据**：若题目要求根据计算结果判定等级（如偏离度等级、安全等级），**必须在题干中明确列出具体的数值阈值表**（例如：“规定：F < 1.0 为低估，1.0 ≤ F < 1.1 为正常，F ≥ 1.1 为高估”），严禁使用“按既定规定”这种模糊表述代替具体数值。
          - **支持链式推理**：允许描述 A -> B -> C 的多步推导逻辑。不要为了简化而牺牲逻辑的严密性。如果需要先计算中间变量再查表，请完整描述这一过程。
          - **自包含性**：题目必须包含所有必要的常量定义和公式，确保读者仅凭题干和图片即可得出唯一确定的答案，无需猜测隐藏设定。
        {previous}
...
{operate_calculation_block}
        ...
        - {cross_modal}
        - 题干必须围绕图片中心视觉锚点，禁止出现“文献”“文档”“上下文”“context”“结合文献”“依据文献”“文中”“原文”“文本内容”“参考信息”等字样，且不得出现图号/图表编号（如“图1”“图2”“图10”“图X”“图表1”“如图X”）。
        - 风格硬约束：
          - 题干正文不得出现“【】”以及“已知条件/判据/任务说明/提示/步骤/解题思路”等词。
          - 不得使用(1)(2)(3)或“先…再…”等指令式引导。
...
def build_graph_1hop_step_prompt(
    *,
    ...
    return dedent(
        ...
        - {cross_modal}
        - 题干中禁止出现“文献”“文档”“context”“文中”“原文”“文本内容”“参考信息”等字样，且不得出现图号/图表编号（如“图1”“图2”“图10”“图X”“图表1”“如图X”）。
