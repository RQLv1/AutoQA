...
def build_stage2_step_prompt(
    ...
    return dedent(
        ...
        - 你必须将 fact_hint 改写为题干中的【已知条件/判据/公式/参数】(至少1条)，不得提及来源。
...
{operate_calculation_block}
        - 你必须在题干中写入 operate_calculation 草稿里的“题干条件”句子，可微调措辞但保留判据/公式/阈值。
        - {cross_modal}
        - 题干包含 A-D 选项，答案需可验证。
...
def build_graph_1hop_step_prompt(
    *,
    ...
    return dedent(
        ...
        - 你必须将参考证据改写为题干中的【已知条件/判据/公式/参数】(至少1条)，不得提及来源。
        - 你必须在题干中写入 operate_calculation 草稿里的“题干条件”句子，可微调措辞但保留判据/公式/阈值。
        ...
        参考证据(仅供内部推理；允许改写为题干中性条件/阈值/公式，不得提及来源):
