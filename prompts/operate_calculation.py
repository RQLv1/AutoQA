...
def build_operate_calculation_prompt(
    *,
    ...
    return dedent(
        f"""
        你是 operate_calculation 智能体。你的任务不是出题，而是为“下一步子问题”生成一个可执行的修改草稿(draft)，
        ...
        - 必须在两种逻辑模板中二选一来组织计算：
          1) 双源合成(Synthesis)：提取图中读数 X + 参考信息系数/阈值 Y，通过计算得到结论（如 X*Y、对照阈值分级）。
          2) 条件分支(Branching)：参考信息给出分支条件（如 温度>50 用规则A 否则B），通过视觉观察选择分支后再计算/判级。
        ...
        <draft>
        用要点描述（不要写完整题干）：
        1) 视觉证据：图中哪些数字/刻度/相对关系/计数可用于计算？
        2) 参考信息：使用哪条公式/阈值/表格数据？（可概括，不要原文整段复制）
        3) 计算：列出关键步骤，给出正确结果（带单位/区间/等级），并说明取整/误差规则
        4) 选项：A-D 候选（数值/区间/等级），其中 3 个为 Hard Negatives（边界、单位换算错、读图错等）
        </draft>
        """
    ).strip()
